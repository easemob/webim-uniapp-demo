  ...
  methods: {
    ...
    _checkScrolledToBottom(scrollDiff, checked = false) {
      if (this.cacheScrollNodeHeight === -1) {
        ...
        if (
          scrollDiff - this.cacheScrollNodeHeight <=
          this.finalLowerThreshold
        ) {
          this._onLoadingMore('toBottom');
        } else if (scrollDiff - this.cacheScrollNodeHeight <= 500 && !checked) {
          u.delay(
            () => {
              this._getNodeClientRect('.zp-scroll-view', true, true).then(
                (res) => {
    ...
    _onLoadingMore(from = 'click') {
      ...
      if (!this.isIos && !this.refresherOnly && !this.usePageScroll) {
        ...
        if (
          ...
          currentTimestamp - this.loadingMoreTimeStamp < 100
        ) {
          this.loadingMoreTimeStamp = 0;
          return;
        }
    ...
    _preCheckShowNoMoreInside(newVal, scrollViewNode, pagingContainerNode) {
      if (
        this.loadingStatus === Enum.More.NoMore &&
        this.hideNoMoreByLimit > 0 &&
        newVal.length
      ) {
        this.showLoadingMore = newVal.length > this.hideNoMoreByLimit;
      } else if (
        (this.loadingStatus === Enum.More.NoMore &&
          this.hideNoMoreInside &&
          newVal.length) ||
        (this.insideMore && this.insideOfPaging !== false && newVal.length)
      ) {
        this.$nextTick(() => {
          this._checkShowNoMoreInside(
            newVal,
            scrollViewNode,
            pagingContainerNode
          );
    ...
    async _checkShowNoMoreInside(
      ...
      try {
        const scrollViewNode =
          oldScrollViewNode ||
          (await this._getNodeClientRect('.zp-scroll-view'));
        if (this.usePageScroll) {
          if (scrollViewNode) {
            const scrollViewTotalH =
              scrollViewNode[0].top + scrollViewNode[0].height;
          ...
          const pagingContainerNode =
            oldPagingContainerNode ||
            (await this._getNodeClientRect('.zp-paging-container-content'));
          const pagingContainerH = pagingContainerNode
            ? pagingContainerNode[0].height
            : 0;
    ...
    _showLoadingMore(type) {
      ...
      const res =
        this.loadingStatus === Enum.More[type] &&
        this.zSlots[`loadingMore${type}`] &&
        (type === 'NoMore' ? this.showLoadingMoreNoMoreView : true);
      if (res) {
        // #ifdef APP-NVUE
        if (!this.isIos) {
