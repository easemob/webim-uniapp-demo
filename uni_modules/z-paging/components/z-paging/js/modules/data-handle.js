...
export default {
  ...
  methods: {
    ...
    addData(data, success = true) {
      ...
      const addDataDalay =
        this.requestTimeStamp > 0 && disTime < minDelay
          ? minDelay - disTime
          : 0;
      this.$nextTick(() => {
        u.delay(
    ...
    addChatRecordData(data, toBottom = true, toBottomWithAnimate = true) {
      ...
      //#endif
      //#ifdef APP-NVUE
      this.totalData = this.nIsFirstPageAndNoMore
        ? [...this.totalData, ...data]
        : [...data, ...this.totalData];
      //#endif
      if (toBottom) {
    ...
    refresh() {
      return this._handleRefreshWithDisPageNo(
        this.pageNo - this.defaultPageNo + 1
      );
    },
      ...
      if (this.finalUseCache && this.totalData.length) {
        this._saveLocalCache(
          this.totalData.slice(
            0,
            Math.min(this.totalData.length, this.pageSize)
          )
        );
      }
    },
    ...
    _preReload(animate = this.showRefresherWhenReload, isFromMounted = true) {
      ...
      if (animate) {
        ...
        u.delay(
          () => {
            this._getNodeClientRect('zp-n-refresh-container', false).then(
              (node) => {
                if (node) {
                  ...
                  u.delay(() => {
                    this._nDoRefresherEndAnimation(
                      0,
                      -nodeHeight,
                      false,
                      false
    ...
    _reload(isClean = false, isFromMounted = false, isUserPullDown = false) {
      ...
      !this.privateShowRefresherWhenReload &&
        !isClean &&
    ...
    _addData(data, success, isLocal) {
      ...
      if (this.isFirstPage) {
        ...
        if (
          this.finalUseCache &&
          success &&
          (this.cacheMode === Enum.CacheMode.Always
            ? true
            : this.isSettingCacheList)
        ) {
      ...
      if (success) {
        ...
        if (isLocal) {
          ...
          if (
            this.privateShowRefresherWhenReload &&
            this.finalNvueListIs === 'waterfall'
          ) {
        ...
        if (this.isHandlingRefreshToPage) {
          ...
          this.pageNo =
            this.defaultPageNo + Math.ceil(data.length / this.pageSize) - 1;
    ...
    _totalDataChange(newVal, oldVal, eventThrow = true) {
      ...
      this.$nextTick(() => {
        u.delay(() => {
          this._getNodeClientRect('.zp-paging-container-content').then(
            (res) => {
    ...
    _currentDataChange(newVal, oldVal) {
      ...
      if (this.customNoMore !== -1) {
        if (
          this.customNoMore === 1 ||
          (this.customNoMore !== 0 && !newVal.length)
        ) {
          this.loadingStatus = Enum.More.NoMore;
        }
      } else {
          ...
          if (this.pageNo !== this.defaultPageNo) {
            ...
            this.$nextTick(() => {
              this._scrollIntoView(
                idIndexStr,
                30 + Math.max(0, this.cacheTopHeight),
                false,
                () => {
    ...
    _localPagingQueryList(pageNo, pageSize, localPagingLoadingTime, callback) {
      ...
      const finalPageNoIndex = Math.min(
        totalPagingList.length,
        pageNoIndex + pageSize
      );
      const resultPagingList = totalPagingList.splice(
        pageNoIndex,
        finalPageNoIndex - pageNoIndex
      );
    ...
    _emitQuery(pageNo, pageSize, from) {
      ...
      this.$emit(
        'query',
        ...interceptor._handleQuery(pageNo, pageSize, from, lastItem || null)
      );
    },
    //触发数据改变promise
    _callDataPromise(success, totalList) {
      for (const key in this.dataPromiseResultMap) {
        ...
        success
          ? obj.resolve({
              totalList,
              noMore: this.loadingStatus === Enum.More.NoMore,
            })
          : this.callNetworkReject && obj.reject(`z-paging-${key}-error`);
      }
    },
    //检查complete data的类型
    _checkDataType(data, success, isLocal) {
      ...
      if (dataType === '[object Boolean]') {
        ...
        if (dataType !== '[object Undefined]' && dataType !== '[object Null]') {
          u.consoleErr(
            `${
              isLocal ? 'setLocalPaging' : 'complete'
